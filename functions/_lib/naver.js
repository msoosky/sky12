...
export async function getPrice(stockCode) {
  ...
  const res = await fetch(`https://m.stock.naver.com/api/stock/${stockCode}/basic`, { headers: HEADERS })
  ...
  await cachePut(cacheKey, result, 30) // 시세는 짧게 캐싱
  return result
}
...
export async function getChartRange(stockCode, startYear = EARLIEST_CHART_YEAR, endYear = new Date().getFullYear()) {
  ...
  const res = await fetch(`https://api.finance.naver.com/siseJson.naver?${params}`, { headers: HEADERS })
  ...
  await cachePut(cacheKey, rows, 60 * 60) // 과거 시세는 거의 안 바뀌므로 1시간 캐싱
  return rows
}
